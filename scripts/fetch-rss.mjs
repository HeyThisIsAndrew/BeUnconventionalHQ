...
async function run() {
  ...
  try {
    ...
    if (contentType.includes('text/html')) {
      throw new Error(
        `Substack WAF block detected: Received text/html instead of XML.`
      );
    }
    ...
    if (!xml.trim().startsWith('<?xml') && !xml.includes('<rss')) {
      console.error(
        '[RSS] FATAL: Received invalid payload (WAF block or HTML challenge). Snippet:'
      );
    ...
    console.log(
      `[RSS] Successfully parsed and cached ${normalizedArticles.length} articles to ${CACHE_FILE}`
    );
  } catch (error) {
    ...
    try {
      ...
      console.log(
        `[RSS] Fallback: No cache file exists. Writing empty array to prevent build crash.`
      );
